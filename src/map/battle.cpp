...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if( sc && sc->count ) { // SC_* that reduce damage to 0.
		if( sc->data[SC_BASILICA] && !status_bl_has_mode(src,MD_STATUS_IMMUNE) && skill_id != SP_SOULEXPLOSION ) {
			d->dmg_lv = ATK_BLOCK;
			return 0;
		}
...
static void battle_calc_skill_base_damage(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch (skill_id) {	//Calc base damage according to skill
		...
		case NC_SELFDESTRUCTION: {
				int damagevalue = (skill_lv + 1) * ((sd ? pc_checkskill(sd,NC_MAINFRAME) : 0) + 8) * (status_get_sp(src) + sstatus->vit) * 2;
	...
	switch (skill_id) {
		...
		case SR_GATEOFHELL: {
			...
			if(sc && sc->data[SC_COMBO] && sc->data[SC_COMBO]->val1 == SR_FALLENEMPIRE) {
				...
				ATK_ADD(wd.damage, wd.damage2, (sp_bonus + hp_bonus) + (40 * status_get_lv(src)));
			} else {
				int64 sp_bonus = (sstatus->sp * sp_xbonus);
				ATK_ADD(wd.damage, wd.damage2, (sp_bonus + hp_bonus) + (10 * status_get_lv(src)));
			}
		}
		break;
	}
...
// rAthena
	...
	{ "reserved_costume_id",                &battle_config.reserved_costume_id,             1,      0,      INT_MAX,        },
	...
	{ "exp_cost_inspiration",               &battle_config.exp_cost_inspiration,            0,      0,      1,				}, //custom value

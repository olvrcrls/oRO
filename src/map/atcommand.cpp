...
ACMD_FUNC(guildskill)
{
	...
	if (!map_getmapflag(sd->bl.m, MF_GVG))
	{
		clif_displaymessage(fd, "This command is only available for GvG.");
		return -1;
	}
	else
	{
		...
		{
			if (gd->skill[skillnum].id == INVALID_TIMER) // ToDO need to change this
				skilllv = gd->skill->lv;
			else
			{
				guild_block_skill(sd, skill_get_time2(gd->skill->id, skilllv));
				skilllv = 0;
			}
		}
		else
		{
			clif_displaymessage(fd, "This command is reserved for Guild Masters Only.");
			return -1;
		}
	}
